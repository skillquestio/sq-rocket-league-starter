...
def find_hits(agent, targets):
    ...
    while i < struct.num_slices:
        ...
        if time_remaining > 0:
            ...
            forward_flag = forward_time > 0.0 and (distance*1.025 / forward_time) < (
                2299 if agent.me.boost > distance/100 else max(1400, 0.8 * agent.me.velocity.flatten().magnitude()))
